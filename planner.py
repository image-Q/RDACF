...
class Planner(object):
    def __init__(self, predictor=None, use_interaction=False, render=False):
        self.horizon = 3
        self.dt = 0.1
        self.cost_weights = np.array([1, 0.5, 0.5, 0.5, 1, 3, 5])
        self.use_interaction = use_interaction
        self.render_obs = render
        self.predictor = predictor
        self.device = next(
            predictor.parameters()).device
    ...
    def generate_routes(self, paths):
        ...
        for path in paths:
            ...
            while len(path) < 51:
                ref_wp = path[-1]
                path.append(Waypoint(pos=np.array([ref_wp.pos[0] + np.cos(ref_wp.heading + np.pi / 2),
        ...
        self.target_route = np.min(
            [np.min(np.linalg.norm(route[:, :2] - goal[None, :2], axis=-1)) for route in self.routes])
        return cs_routes
    ...
    def generate_emergency_trajectory(self, route, cs_route):
        ...
        current_d = signed_dist_to_line(self.ego_state.position[:2], route[0, :2],
    ...
    def generate_trajectory_on_frenet(self, acc, route):
        ...
        fp = FrenetPath()
        fp.t = np.arange(self.dt, self.horizon + self.dt, self.dt)  # 时间向量
    ...
    def get_other_agent_reaction(self, env_input, plan):
        ...
        # query the model
        env_input = {key: torch.as_tensor(_obs).unsqueeze(0).float().to(self.device) for (key, _obs) in
                     env_input.items()}
        with torch.no_grad():
    ...
    def calculate_cost(self, plan, prediction, route):
        ...
        base_costs = np.array([d2g, speed, lon_jerk, lat_acc, d2a, ttc, collision])

        traffic_density = min(len(prediction) / 5.0, 1.0)
        ego_speed = np.linalg.norm(self.ego_state.linear_velocity[:2])
        speed_factor = np.clip(ego_speed / self.speed_limit, 0.0, 2.0)

        base_weights = np.array([1, 0.5, 0.1, 0.1, 1, 5, 10])
        alpha_risk = 0.4
        alpha_comfort = 0.2
        dynamic_weights = base_weights.copy()
        dynamic_weights[4:7] *= (1 + alpha_risk * traffic_density)
        dynamic_weights[2:4] *= (1 - alpha_comfort * (1 - speed_factor))


        gamma_base = 0.85
        gamma = np.clip(gamma_base + 0.1 * speed_factor, 0.7, 0.95)

        severity_scale = 2.0
        T = min(len(plan.x), len(plan.y), 30)
        ...
        for t in range(T):
            min_dist = 100.0
            ego_pos_t = np.array([plan.x[t], plan.y[t]])
            for a in prediction:
                if t < len(a):
                    agent_pos_t = a[t, :2]
                    dist = np.linalg.norm(ego_pos_t - agent_pos_t)
                    min_dist = min(min_dist, dist)

            dist_list.append(min_dist)

            prob_collision = np.exp(-0.1 * (min_dist ** 2))
            severity = np.exp(-0.05 * min_dist) * severity_scale
            risk_t = (gamma ** t) * prob_collision * severity
            risk_term += risk_t

            max_risk_t = (gamma ** t) * 1.0 * severity_scale  # exp(0)=1
            max_possible_risk += max_risk_t

        if max_possible_risk > 1e-6:
            risk_term = risk_term / max_possible_risk
        else:
            risk_term = 0.0

        if getattr(self, "render_obs", False):
            save_path = f"./training_log/risk_plot/episode_{np.random.randint(10000)}.png"
            self.visualize_risk_propagation(
                risk_list=[(gamma ** t) * np.exp(-0.1 * (d ** 2)) for t, d in enumerate(dist_list)],
                dist_list=dist_list,
                gamma=gamma,
                save_name=save_path
            )
        ...
        w_risk = 0.5
        cost = np.sum(base_weights * base_costs) + w_risk * risk_term
    ...
    def plan(self, obs, env_input, training=False):
        ...
        if training:
            self.cost_weights[-3:] = np.zeros(shape=(3,))  # array([1, 0.5, 0.1, 0.1, 0, 0, 0])
        else:
            self.cost_weights[-3:] = np.array([1, 5, 10])  # array([1, 0.5, 0.1, 0.1, 1, 5, 10])
        ...
        for d, t in trajectories.items():
            ...
            prediction = self.project_to_world_position(prediction_ego_frame, self.ego_state.position,
    ...
    def check_comfort(self, traj):
        ...
        v_x, v_y, theta = np.diff(traj[:, 0]) / 0.1, np.diff(traj[:, 1]) / 0.1, traj[1:, 2]  # x方向速度，y方向速度，偏航角
